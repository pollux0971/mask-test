...
/meta (attrs)
    ...
    sensors_enabled_confirmed,  # bool，選填，跟 sensors_enabled 成對；幾乎
                                 ...
                                 # 不是裝置確認過的狀態
    sensors_seen,           # "" | "A" | "B" | "AB"，選填；見下方三個欄位
                            # 的差別說明，"" 是合法值，跟 attr 不存在意義相反
    ...
    energy_mu, energy_sigma  # 選填，成對；唇動偵測的能量門檻改用 baseline
                             # 當下的真實幀算，不再用估的（B16：估的偏嚴
                             # 約 23%，會讓 lip_onset_us 偏晚）
...
>
> 🔴 **三個感測器狀態欄位問的是三個不同的問題，不可互相取代——三者可以
> 互相不一致，而那個不一致本身就是診斷用的資訊。**
>
> | 欄位 | 問的問題 | 來源 |
> |---|---|---|
> | `sensors_enabled` | **指令設成什麼**（主機端最後一次下達的） | `SENS:` 指令 |
> | `sensors_enabled_confirmed` | **裝置有沒有確認收到指令** | `$STATUS`（目前恆為 `False`，見上） |
> | `sensors_seen` | **實際有沒有資料流過來**（線上出現過哪些標籤） | `bridge_server.py` 對資料流本身的統計 |
>
> `sensors_seen` 是真板子第一次上線時發現需要的：`sensors_enabled`／
> `sensors_enabled_confirmed` 都測不出「韌體回報活著、但這個 session
> 期間資料流其實沒送過來」這類落差——`$H` 的 `drop_A`/`drop_B` 只統計
> 讀取失敗次數，不是「根本沒在讀」；`$STATUS` 沒有欄位能說哪幾顆真的在跑。
>
> ⚠️ **`sensors_seen = "A"` 的意思是「線上出現過標著 A 的 `$T` 行」，
> 不是「A 這顆實體感測器是好的」**——主機端只看得到協定裡的標籤，看不到
> 韌體那一側是哪一顆晶片在送資料。不要把這個欄位簡化成「哪幾顆感測器
> 有作用」，它誠實的語意只到「看到幾條資料流、它們自稱是誰」為止。
>
> 🔴 **`""` 是合法值，跟「這個 attr 不存在」意義相反，不可互相取代：**
>
> | 值 | 意思 |
> |---|---|
> | `"AB"` / `"A"` / `"B"` | 監測到對應的資料流 |
> | `""` | **監測過整個 session，確實沒有任何感測器的資料流過來**——已知的結論，而且是很糟的那種 |
> | attr 不存在 | 舊檔，這個欄位當時還不存在——**未知**，不是「零」 |
>
> 把 `""` 攤平成「attr 不存在」，會讓「這批資料是廢的」偽裝成「這只是接
