...
> **`vad_start_us` / `vad_end_us` / `lip_onset_us` / `voice_onset_us`
> 偵測不到時一律填 `None`，不可填 `0` 或 capture 視窗邊界。**
>
> 填視窗邊界會讓「**完全沒偵測到**」看起來像「**整段都在動**」——
...
> ⚠️ **`sigma` 下限 `1e-3` 只適用於「已正規化的特徵」。**
> 直接對**原始整數 mm 距離**做 z-score 時，下限必須是**量化尺度**
> `1/√12 ≈ 0.289 mm`，不是 `1e-3`。
>
> **為什麼**：`$T` 的距離是整數 mm。靜止時一個回波極穩的 zone
> 幾乎每幀都輸出同一個整數 → σ 趨近 0 → z 可以爆到 **10⁵–10⁶ 量級**，
> 那一個 zone 會單獨主宰整條能量訊號，**而且不會有任何錯誤訊息**。
> 一個量化到 1 mm 的量測，其雜訊 σ 不可能有意義地小於量化本身。
> （`B16` 實測 mock 的靜止串流：baseline 17.0 ± 0.15 mm，輸出後 z 爆到 9.4×10⁵。
> **真機上任何貼著硬表面的 zone 都會踩到。**）
>
> **同理，MAD 在高度量化的資料上會剛好回傳 0**（超過一半的幀是同一個整數），
> 門檻會塌成 `mu + 3×1e-3`，幾乎任何東西都會觸發。
> σ 低於理論下界時應夾到理論下界並記錄原因——
> **低於理論下界代表估計器失效，不是「資料超級乾淨」**。
