...
| 發現 | 嚴重度 | 歸誰 | 狀態 |
|---|---|---|---|
| trial 提示詞永遠不換（存檔、abort 都無效） | 🔴 擋住整條路 | `state_machine.py`／`record.js`（`4f`） | 回報，已指派，未修 |
| 表單「每詞目標筆數」欄位沒接上選字邏輯 | 🔴 可能同根因 | 同上 | 回報，已指派，未修 |
| `quiz.js` 把 `/recognize` 的任何非 200 回應都顯示成「還沒上線」 | 🔴 第 6 個「前端沒跟上」案例 | `quiz.js`（owner 待調度員指派） | 回報，未修（不在授權範圍） |
