...
### 1.2 `B20` bridge 掉幀率 → 決定 `E05` 能不能開始錄

**誰在等**：`E05`（4 小時主資料集蒐集）的資料品質。

`B19` 實測：本機 pty、**只用 18% 頻寬就掉 0.7% 的行**。
...
⚠️ **`bw_bytes_since_last` 目前只涵蓋 `$T`/`$STATUS`/`$H`/`$M`/`$F`**，
錄音 dump（`uart_cmd.c`）還沒接——**是下界不是總量**。
`fw_regression.py` 量的是序列埠層實收，兩者可互相對照。
...
- [ ] **「詞」與「靜止」的錄製次數要對齊。** `D06` 實測：
      `_reject` 樣板數量多於詞樣板時，誤拒率從 7.33% 升到 14.67%——
      因為樣板越多、兩兩最近距離的期望值越小，閾值被拉得太緊。
      次數差很多的話，`fit_reject_threshold` 的 percentile 要調。
