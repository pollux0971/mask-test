...
## 0.5 🔴 感測器 B 完全沒資料（`fw=22bddce` 實測，正在發生）

**現象**：`$T,A` 約 30Hz 正常送、`$T,B` 是 **0 行**、`$H` 的 `drop_B` 是 **0**
（不是在累加，是韌體根本沒在讀 B，連「讀失敗」都沒發生過）。

### 為什麼 `drop_B=0` 不代表「B 沒事」

`main/vl53l7cx_test.c:337` 開機時跑
`ok[i] = init_bus_and_sensor(i)`；**只要 `init_bus_and_sensor()` 對 B 回傳
`false`，主迴圈（`vl53l7cx_test.c:372`）就會對 B 永遠 `continue`
——SENS 指令、data-ready 輪詢、drop 計數、`$T,B` 全部被跳過，
**永遠不會再重試**，不是暫時性的。`drop_B` 的計數邏輯就在這個被跳過的
區塊裡面，所以它會停在 0，**跟「B 讀取一直成功」長得一模一樣，
必須靠 `$T,B` 有沒有行數來分辨，不能只看 `drop_B`**。
...
硬體設計下不成立**——沒有共用匯流排，也沒有 XSHUT/LPn 腳位這回事（`pins[]`
裡只有 SCL/SDA，沒有 shutdown pin）。B 沒資料的原因**只可能在 B 自己那條
`I2C_NUM_1`（GPIO7=SCL、GPIO6=SDA）這條線或那顆模組本身**，跟 A 那條無關。
...
情形**——B 現在的問題有沒有可能是同一類「哪支是 SDA、哪支是 SCL 兜錯」，
只是這次剛好只有 B 那邊還沒兜對，值得優先懷疑。

### 韌體會不會「安靜地只跑 A」？——**會，而且現在看不出來是不是這樣**

`init_bus_and_sensor()` 失敗時**只在開機當下印一行 `ESP_LOGE`**（見下表），
之後**完全沒有任何持續性的提示**——不會反覆警告、`$STATUS` 沒有欄位講
「B 未初始化」、`SENS:B=1` 指令送下去也只會印 `SENS:B=1 (on)`
（`uart_cmd.c:65`，**這行不檢查 B 到底有沒有初始化成功，指令看起來「被接受了」
但實際上從來沒有被套用到任何東西**，因為 `app_main` 主迴圈裡 B 那塊已經被跳過，
不會去讀這個指令）。

🔴 **這是一個需要修的缺口，但這次不動韌體**（板子是活的）：使用者現在唯一能
確認 B 有沒有初始化成功的辦法，是**回頭找開機當下那幾行 log**，序列埠上
之後不會再出現任何相關訊息。

### 🔴 一分鐘檢查表

1. **回頭找開機時（`$STATUS` 出現前後那幾秒）的 log，找 `[Sensor B]` 開頭的行**
   ——`idf.py monitor` 的 scrollback 往上滑，或用終端機的搜尋功能找 `Sensor B`。
   哪一行出現，直接對應下面哪一段程式碼失敗：

   | 看到的字串 | 失敗在哪一步 | 意思 |
   |---|---|---|
   | `[Sensor B] i2c_new_master_bus failed: ...` | 建立 I2C 匯流排本身就失敗 | GPIO6/7 設定衝突或腳位被別的東西占用，**最不像是這次的情形**（A 也用同一組 API 沒事） |
   | `[Sensor B] i2c_master_bus_add_device failed: ...` | 掛上裝置失敗 | 同上，罕見 |
   | **`[Sensor B] is_alive failed (status=%u alive=%u)`** | **I2C 匯流排建好了，但 ping 不到感測器晶片** | 🔴 **最可能是這個**——通常代表 GPIO6(SDA)/GPIO7(SCL) 沒接對、沒接上，或模組沒通電。**先查這個** |
   | `[Sensor B] vl53l7cx_init failed, status=%u` | ping 得到，但上韌體失敗 | I2C 通了但不穩定（雜訊、線太長、內部上拉電阻在 400kHz 下勉強），或模組本身壞了 |
   | `[Sensor B] configuration failed` / `start_ranging failed` | 上韌體成功，設解析度/開始量測失敗 | 少見，通常代表前面其實已經半成功 |

   **沒看到任何 `[Sensor B]` 的行** → 代表 `ok[1]` 其實是 `true`（B 有初始化成功），
   那 B 沒資料的原因不在初始化，要往別處查（例如韌體之後某處把 B 的輸出關掉了
   ——但目前程式碼沒有這種路徑，這種情況不太可能發生）。

2. **檢查板子上 Sensor B 那組接線**：對照 `GPIO7 = SCL_B`、`GPIO6 = SDA_B`
   （軟體已經照上面那條「SDA/SCL 對調」的註解接好，**不要自己再交換一次**，
   照這個對應查實際接線）。確認：
   - 這兩條線真的接到 B 模組的 SCL/SDA 腳，不是接錯到別的地方或懸空
   - B 模組的電源（VDD/GND）真的有接上、電壓正常——`is_alive` 失敗
     韌體端沒辦法分辨「線沒接」還是「沒通電」，這兩種都要排除
   - A 那組（GPIO5=SCL_A、GPIO4=SDA_A）目前正常，可以拿來對照「正確的接法長怎樣」

3. **兩條內部上拉電阻都是軟體開的（`enable_internal_pullup=true`），
   A/B 設定一樣**，所以如果只有 B 出問題，不太可能是上拉電阻設定差異，
   優先懷疑接線本身而不是程式碼配置。
