...
### 🔴 這不是「錄幾次」的問題，是**校準方法本身的結構性限制**

`fit_reject_threshold()` **只用 `_reject` 類別自己的 leave-one-out 距離分布**
來校準門檻。但「LOO 最近距離」與「真詞查詢到自己樣板的最近距離」
**在統計上是同一種量**（都是「同噪音分布下的最近鄰距離」）——
**樣板數增加時兩者會用類似的速度一起縮小**，門檻不會相對變寬。

**這直接影響 Demo 腳本第 4 步**（純 ToF 念「四」→ 認不出來）。
32% 的誤拒率代表三次裡有一次連正確的詞都會被拒絕。

→ **已開 `D22` 重新設計校準方法**（用 ROC 在「真詞距離」與
「拒識距離」兩個分布上取門檻，而不是只看單邊）。

### 錄製次數的暫定結論

- [ ] **樣板數不是瓶頸**，但仍建議每類 **20–30 次**
      （足夠讓 LOOCV 有統計意義，再多是浪費你的時間）
- [ ] 「詞」與「靜止（`_reject`）」的次數**仍然要對齊**——
      `D06` 實測不對齊時誤拒率從 7.33% 升到 14.67%，那個效應是真的
