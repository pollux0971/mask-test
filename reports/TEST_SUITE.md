...
### ✅ `vl53l7cx_test/monitor/`：**159 passed, 0 failed, 0 errors**（681s）

```bash
.venv/bin/python3 -m pytest vl53l7cx_test/monitor/ -q
# 13:08:30 開始，load average 1.98 → 13:19:51 結束，681.36s
```

⚠️ **這個數字是在一個「約定的」barrier 下拿到的，不是「找到的空窗」**
——見下一節，那個區別花了一整天才學會。

從同日稍早的 **123** 漲到 **159**（+36）：當天下午新增了
`test_bridge_device_api.py` 等檔案（裝置掃描／連接、`figures`/`extras`、
`session_summary`、串流逾時、`sensors_seen` 三層、驗證介面三階段）。

---

## 🔴 為什麼拿這一個數字花了一整天：三個層面的同一個問題

**這一節不是抱怨，是給下一個人的。** 今天為了拿到上面那個數字，
花掉的不是 CPU 是**協調成本**——而三次卡關的根因是三件不同的事。

### 層面一：排程 —— 「空窗」是**觀測**，不是**約定**

嘗試了兩次「等一個空窗再跑」，兩次都失敗：

| 時間 | load（8 核） | 發生什麼 |
|---|---|---|
| 12:03 | 6.56 | 有人在跑同一個目錄 → 不敢跑 |
| 12:35 | **3.49** ✅ | 開了窗口，但**正在跑的就是這個目錄最敏感的三個檔案** → 設有界等待 |
| 12:55 | **9.00** 🔴 | **另一個人開了新的一輪** → 等待條件永遠成立，永遠等不到 |

> 🔴 **「現在沒人跑」不代表「接下來十分鐘沒人跑」。**
> **空窗是一個瞬時觀測，而這個目錄需要一段連續 10 分鐘的獨佔。**

**正確的做法是 barrier**：由協調者**明確要求大家讓出一段時間並取得確認**，
然後才開跑。**前者是被動觀測，後者是主動約定。**

⚠️ **這個區別看起來很細，但它決定了成敗**——同一個人用「找空窗」失敗兩次，
改用「取得承諾」一次就成。

### 層面二：架構 —— 這個目錄**本來就不該用「找空窗」跑

`vl53l7cx_test/monitor/` 的每個測試都起**一對子行程**（mock device + bridge）。
159 個測試 = **約 90 對行程、11 分鐘獨佔**。

> **一個需要獨佔 10 分鐘、起 90 對子行程的測試目錄，
> 在多 agent 環境下沒有辦法靠「找空窗」跑到。**
> **那是測試套件的架構問題，不是排程問題。**

**兩個方向（只回報，未實作）**：

1. **降低子行程成本** —— 例如 module-scoped 的 `Rig` fixture，
   讓一個 bridge 服務多個測試（現在每個測試各起一對）
2. **承認它需要獨佔** —— 標成 `@pytest.mark.serial`，日常不跑，
   只在明確的 barrier 下跑

⚠️ **兩個都會動到 `Rig`（`test_bridge_sse.py`）**，所以**沒有實作**。

### 層面三：識別 —— 🔴 **「這個行程是不是我的」在多 agent 環境下沒有可靠判準**

**這一面最值得記，因為它造成的是實際的破壞**（一輪測試被中途殺掉）。

`Rig` 建立的沙箱目錄前綴是 `bridge-test-`。有人拿它當「這是我的行程」的
判準去清理，結果**殺掉了另一個 agent 正在跑的 rig**。

> **`bridge-test-` 這個前綴是設計來標示「這是測試沙箱」，
> 不是「這是我的」。**
> **而它被當成後者用了——在只有一個人跑的時候，兩者剛好等價。**

🔴 **「窄一點的 pattern kill 仍然是 pattern kill。」**

**正確做法**：只終止**自己 `Popen` 出來的子行程**（`Rig.close()` 本來就做了），
**收工時不要再用 pattern 去掃**。要確認殘留時用
`ps -eo pid,lstart,args` **看啟動時間**，只清自己那一輪的。

⚠️ **`pgrep -f "..."` 會匹配到自己**——本文件作者在檢查殘留時就抓到自己的
`pgrep` 子殼，差點把它當成孤兒。用 `ps -eo` 並排除 `grep` 自己。
